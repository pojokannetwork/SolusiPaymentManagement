<?php
$page_title = 'Gudang - Pengeluaran / Mutasi';
...
<div class="p-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="mb-0">Pengeluaran / Mutasi</h2>
        <div class="d-flex gap-2">
            <button class="btn btn-outline-secondary" id="btn-export" disabled>
                <i class="fas fa-file-export me-2"></i>Export
            </button>
            <button class="btn btn-primary" id="btn-add-issue">
    ...
    <div class="card mb-3">
        <div class="card-body">
            <div class="row g-3">
                <div class="col-md-3">
                    <input type="text" id="search" class="form-control" placeholder="Cari no. dokumen / tujuan">
                </div>
                <div class="col-md-3">
    ...
    <div class="card">
        <div class="card-header"><h5 class="mb-0"><i class="fas fa-outdent me-2"></i>Daftar Pengeluaran / Mutasi</h5></div>
        <div class="card-body">
            <div class="table-responsive">
                <table class="table align-middle" id="tbl-issues">
                    ...
                    <tbody>
                        <tr>
                            <td colspan="6" class="text-center py-4 text-muted">Belum ada data pengeluaran/mutasi.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="issModalTitle">Tambah Pengeluaran/Mutasi</h5>
      ...
      <div class="modal-body">
        ...
        <div class="row g-3 mb-3">
            <div class="col-md-4">
                ...
                <input type="date" class="form-control" id="iss-date" value="<?php echo date('Y-m-d'); ?>">
            </div>
            <div class="col-md-4">
                <label class="form-label">No. Dokumen</label>
                <input type="text" class="form-control" id="iss-docno" placeholder="Otomatis jika kosong">
            </div>
            <div class="col-md-4">
                <label class="form-label">Tujuan</label>
                <input type="text" class="form-control" id="iss-target">
            </div>
        </div>
        <div class="table-responsive">
      ...
      <div class="modal-footer">
        ...
        <button type="submit" class="btn btn-primary" id="iss-save-btn"><i class="fas fa-save me-2"></i>Simpan</button>
      </div>
      </form>
    </div>
  </div>
...
document.addEventListener('DOMContentLoaded', ()=>{
    const tbody=document.querySelector('#tbl-issues tbody');
    let modal, editingId=null;
    function render(rows){
        tbody.innerHTML='';
        if(!rows||rows.length===0){ tbody.innerHTML='<tr><td colspan="6" class="text-center py-4 text-muted">Belum ada data pengeluaran/mutasi.</td></tr>'; return; }
        rows.forEach((r,idx)=>{
            const tr=document.createElement('tr');
            tr.innerHTML=`<td>${r.date}</td><td>${r.doc_no}</td><td>${r.target||'-'}</td><td>${r.items||0}</td><td>${r.status||'-'}</td>
            <td class=\"text-end\">
    ...
    function load(){
        fetchJSON('/api/admin/warehouse.php?action=list_issues').then(res=>{
            render(res.issues||[]);
            ensureDataTables(()=>{
                if(!$.fn.DataTable.isDataTable('#tbl-issues')){$('#tbl-issues').DataTable({responsive:true,pageLength:25,order:[]});}
                else{$('#tbl-issues').DataTable().destroy();$('#tbl-issues').DataTable({responsive:true,pageLength:25,order:[]});}
            });
        });
    }
    ...
    function openModal(row){
        ...
        document.getElementById('issModalTitle').textContent = row ? 'Edit Pengeluaran/Mutasi' : 'Tambah Pengeluaran/Mutasi';
        document.getElementById('iss-id').value = row?.id || '';
        document.getElementById('iss-date').value = row?.date || '<?php echo date('Y-m-d'); ?>';
        document.getElementById('iss-docno').value = row?.doc_no || '';
        document.getElementById('iss-target').value = row?.target || '';
        const tbodyLines = document.getElementById('lines-tbody');
        tbodyLines.innerHTML='';
        if(row){
            fetchJSON('/api/admin/warehouse.php?action=get_issue&id='+encodeURIComponent(row.id)).then(r=>{
                const items = r.data?.issue?.items || [];
        ...
        modal = new bootstrap.Modal(document.getElementById('issueModal'));
        modal.show();
    }
    document.getElementById('btn-add-issue').addEventListener('click', ()=> openModal(null));
    document.getElementById('btn-add-line').addEventListener('click', ()=>{ document.getElementById('lines-tbody').appendChild(lineRow({})); });
    document.getElementById('lines-tbody').addEventListener('click', function(e){ const btn=e.target.closest('button'); if(!btn) return; if(btn.classList.contains('btn-remove-line')) btn.closest('tr').remove(); });
    document.getElementById('issueForm').addEventListener('submit', function(e){
        e.preventDefault();
        const payload={ id: document.getElementById('iss-id').value, date: document.getElementById('iss-date').value, doc_no: document.getElementById('iss-docno').value.trim(), target: document.getElementById('iss-target').value.trim(), items: [] };
        ...
        const action = editingId ? 'update_issue' : 'create_issue';
    ...
    tbody.addEventListener('click', function(e){
        ...
        if(btn.classList.contains('btn-del')){
            if(confirm('Hapus dokumen ini?')){
                fetch('/api/admin/warehouse.php?action=delete_issue',{ method:'POST', headers:{'Content-Type':'application/json','X-CSRF-Token':'<?php echo getCsrfToken(); ?>'}, body: JSON.stringify({ id: row.id }) })
        ...
        if(btn.classList.contains('btn-post')){
            if(confirm('Posting dokumen? Stok akan berkurang.')){
                fetch('/api/admin/warehouse.php?action=post_issue',{ method:'POST', headers:{'Content-Type':'application/json','X-CSRF-Token':'<?php echo getCsrfToken(); ?>'}, body: JSON.stringify({ id: row.id }) })
        ...
        if(btn.classList.contains('btn-unpost')){
            if(confirm('Unpost dokumen? Stok akan bertambah.')){
                fetch('/api/admin/warehouse.php?action=unpost_issue',{ method:'POST', headers:{'Content-Type':'application/json','X-CSRF-Token':'<?php echo getCsrfToken(); ?>'}, body: JSON.stringify({ id: row.id }) })
